          ...
          function highlightEvent(event, args)
          {
            ...
            for (var row = 0; row < 6; row += 1)
            {
              for (var date = 0; date < 7; date += 1)
              {
                ...
                rows[row][date].selectedFake = selected;
              }
            }
          }
...
angular.module("template/rcalendar/month.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/rcalendar/month.html",
    ...
    "        </td>\n" +
    "        <td ng-repeat=\"dt in row track by dt.date\" class=\"monthview-dateCell\" ng-click=\"select(dt.date)\" ng-class=\"{'text-center':true, 'monthview-current': dt.current&&!dt.selected&&!dt.hasEvent,'monthview-secondary-with-event': dt.secondary&&dt.hasEvent, 'monthview-primary-with-event':!dt.secondary&&dt.hasEvent&&!dt.selected, 'monthview-selected': dt.selected, 'monthview-selected-fake': dt.selectedFake}\">\n" +
    ...
    "          <td ng-if=\"!event.allDay\" class=\"monthview-eventdetail-timecolumn\">{{event.startTime|date: 'HH:mm'}} - {{event.endTime|date: 'HH:mm'}}\n" +
    "          </td>\n" +
